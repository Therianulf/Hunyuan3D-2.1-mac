    ...
    int width, int height, float occlusion_truncation, int use_depth_prior)
{
    ...
    // (int64_t), not (long): maxint is 0x3FFFFFFF7FFFFFFF, and `long` is 32-bit
    // on Windows (LLP64). Truncating it to 0x7FFFFFFF seeds the z-buffer with
    // depth 0 / face 2147483647, so no triangle ever wins the depth test and the
    // barycentric pass indexes F out of bounds -> "illegal memory access".
